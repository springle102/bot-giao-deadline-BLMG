    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        await interaction.followup.send(
            embed=create_success_embed(
                f"Đã thêm **{count} chap** kèm link riêng biệt cho **{role_name}** - **{self.truyen}**"
            )
        )
...
class AddDeadline(commands.Cog):
    ...
    async def add_deadline_bulk(
        ...
        await interaction.followup.send(
            embed=create_success_embed(
                f"Đã thêm **{count} chap** ({chap_bat_dau}-{chap_ket_thuc}) "
                f"cho **{role_name}** - **{truyen}**"
            )
        )
    ...
    async def add_deadline_single(
        ...
        await interaction.followup.send(
            embed=create_success_embed(
                f"Đã thêm **Chap {chap}** cho **{role_name}** - **{truyen}**"
            )
        )
